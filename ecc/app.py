...
def home():
    ...
    result += "<div>Private key: " + hex(private) + "</div>"
    result += "<div>Public key: (0x{:x}, 0x{:x})</div>".format(*public)
    ...
    result += "<div>======Symmetric key========</div>"

    result += "<div>Encryption key: " + str(S[0]) + "</div>"

    cipher = AESCipher(enc_long(S[0])).encrypt(message)
    result += "<div>Encrypted:\t" + binascii.hexlify(cipher) + "</div>"

    text = AESCipher(enc_long(S[0])).decrypt(cipher)

    result += "<div>Decrypted:\t" + text + "</div>"
    return result
